...
std::vector<std::pair<int, int>> prim(const cv::Mat& img) {
  ...
  std::vector min_w(r / 2, std::vector<int>(c / 2, std::numeric_limits<int>::max()));
  ...
  while(!pq.empty()) {
    ...
    for(int i = 0; i < 4; ++i) {
      ...
      auto cost = adj_edge_cost({id_x, id_y}, {id_nx, id_ny}, img);
      // auto cost = 1;
...
int main() {
  ...
  cv::imshow("resized context base cat.jpg", img2);
  cv::waitKey(0);
  return 0;
}