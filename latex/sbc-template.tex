...
\title{Curvas de Preenchimento de Espaço Guiadas por Dados: \\ Implementação e Avaliação
em Dados Espaço-Temporais}
...
\begin{abstract}
  This work presents an efficient, open-source library that implements and extends the Data-Driven
  Space-Filling Curves method to the temporal domain, introducing alignment techniques to ensure visual continuity between
  frames. Experiments indicate the effectiveness of the alignment in single-object scenarios, despite continuity limitations
  in simultaneous multi-object contexts. Finally, autocorrelation metrics validate the spatial and data coherence trade-offs
  described in the literature.
\end{abstract}
     
\begin{resumo} 
  % A análise visual de dados espaço-temporais é fundamental para a compreensão de fenômenos dinâmicos.
  % Uma das abordagens hoje é baseada na linearização do espaço para criar visualizações estáticas, criando
  % imagens bidimensional onde um eixo representa o espaço linearizado e ou outro o tempo. Formas de linearizar
  % incluem curvas de preenchimento de espaço (\textit{space-filling curves}) e já existem estudos que
  % as usam no contexto espaço-temporal. Contudo, as curvas adotadas neles levam em conta uma geometrica fixa, como
  % a curva de Hilbert. Este trabalho cria uma biblioteca que implementa e estende o método de curvas guiadas por dados
  % \textit{Data-Driven Space-Filling Curves} para o contexto espaço temporal. Essa biblioteca é eficiente, de fácil reuso
  % e adaptação. Neste trabalho também é explorado técnicas de alinhamento temporal para garantir a continuidade visual
  % entre quadros consecutivos. A avaliação da proposta foi realizada por meio de experimentos com animações sintéticas e
  % da aplicação de métricas de autocorrelação, demonstrando a eficácia do método em manter tanto a coerência dos dados
  % quanto a coerência espacial na representação linearizada.
  Este trabalho apresenta uma biblioteca eficiente e de código aberto
  que implementa e estende o método de curvas de preenchimento guiadas por dados (\textit{Data-Driven Space-Filling Curves})
  para o domínio temporal, introduzindo técnicas de alinhamento para garantir a continuidade visual entre quadros. Experimentos
  indicam a eficácia do alinhamento em cenários com um objeto único, apesar de limitações de continuidade em contextos
  com múltiplos objetos simultâneos. Por fim, métricas de autocorrelação validam os compromissos de coerência espacial e
  de dados previstos na literatura.
...
\section{Introdução}
% Os dados espaço-temporais, como o clima de uma região ao longo do tempo ou um padrão que se
% propaga ao longo do tempo, podem ser visualizados de diversas formas. A forma mais comum de analisar
% um dado que vária com o tempo é no formato de um vídeo  ou uma sequência de quadros justapostos. Dados
% postos nessa maneira são de fácil visualização. Porém, para análise, possui pontos negativos, como
% a dificuldade de perceber objetos que aparecem temporariamente e a necessidade do usuário manter contexto
% temporais para analisar a evolução temporal dos dados.

Os dados espaço-temporais, como o clima de uma região ou um padrão de propagação, podem ser visualizados
de diversas formas. A abordagem mais comum para analisar um dado que varia com o tempo é o formato de
vídeo ou uma sequência de quadros justapostos. Embora dados dispostos dessa maneira sejam de fácil
visualização inicial, eles apresentam limitações para tarefas de análise aprofundada.
Dentre os pontos negativos, destacam-se a dificuldade de perceber objetos que aparecem temporariamente
e a necessidade do usuário manter o contexto mental para compreender a evolução temporal dos dados~\cite{PenaAraya:20}.

% Recentemente, houveram abordagens que utilizaram linearizações do espaço para representar estaticamente
% dados temporais~\cite{franke:21, KoeppWeinkauf2022_TMTM}, em especial com curvas de preenchimento de
% espaço (\textit{space-filling curves}).
% A representação estática se dá por uma imagem bidimensional onde
% o eixo vertical representa a linearização de todo espaço, enquanto o eixo horizontal representa o tempo.
% Essa representação podem ser útil em casos que animações não podem ser usadas, como em impressões e em
% como a dificuldade de perceber objetos que aparecem temporariamente e a necessidade do usuário manter contexto
% temporais para analisar a evolução temporal dos dados.

Recentemente, surgiram abordagens que utilizam linearizações do espaço para representar estaticamente dados
temporais~\cite{franke:21, KoeppWeinkauf2022_TMTM}, em especial as baseadas em curvas de preenchimento de espaço (CPE).
As CPEs percorrem todo o espaço da imagem, permitindo que ela seja interpretada de maneira unidimensional, conforme
detalhado na Figura \ref{fig:frame_linearization}. Para dados espaço-temporais, múltiplos quadros são processados
sequencialmente e suas linearizações resultantes são justapostas. O resultado é uma imagem bidimensional onde o eixo
vertical representa o espaço linearizado e o eixo horizontal representa o tempo. Essa representação é particularmente
útil em casos onde animações não são viáveis, como em impressões estáticas.

\begin{figure}[ht]
    \centering
    % Define a altura. Se ainda der erro de overfull, reduza para 5.0cm
    \def\comumheight{5.3cm} 

    % --- Imagem 1 (Quadrada) ---
    % Aumentei para 0.40 para caber a imagem de 5cm+
    \begin{subfigure}[b]{0.40\textwidth} 
        \centering
        \includegraphics[height=\comumheight, keepaspectratio]{images/nucleon_comparison/nucleon_center_slice_2.png}
        \caption{} % Gera apenas (a)
        \label{fig:original_photo}
    \end{subfigure}
    \hfill
    % --- Imagem 2 (Quadrada) ---
    \begin{subfigure}[b]{0.40\textwidth}
        \centering
        \includegraphics[height=\comumheight, keepaspectratio]{images/nucleon_comparison/context_base_traversal_2.png}
        \caption{} % Gera apenas (b)
        \label{fig:spc_traversal}
    \end{subfigure}
    \hfill
    % --- Imagem 3 (Fina) ---
    % Reduzi para 0.15 pois a imagem é muito fina, sobra espaço aqui
    \begin{subfigure}[b]{0.15\textwidth}
        \centering
        \includegraphics[height=\comumheight, keepaspectratio]{images/nucleon_comparison/context_base_frame_visualization_2.png}
        \caption{} % Gera apenas (c)
        \label{fig:linear_visualization}
    \end{subfigure}

    \caption{Processo de linearização de uma imagem: (a) Imagem original; (b) Caminho da CPE; (c) Visualização linearizada disposta verticalmente.}
    \label{fig:frame_linearization}
\end{figure}

% Contudo, as abordagens com curvas de preenchimento de espaço presentes na literatura
% não abordam dados espaço temporais com curvas guiadas por dados.~\cite{dafner:00,zhou:20}. Estas curvas possuem a propriedade de
% balancear a coerência dos dados bem como a coerência espacial, mantendo \textit{features} próximas em sua
% linearização, podendo ser uma abordagem interessante que evite descontinuidades.

Contudo, as abordagens presentes na literatura exploram dados espaço-temporais majoritariamente com CPEs clássicas,
baseadas em padrões geométricos fixos~\cite{franke:21}. Nesse contexto, este trabalho apresenta uma implementação
eficiente, de fácil reuso e adaptação, do método \textit{Data-Driven Space-Filling Curves}~\cite{zhou:20}, expandindo
seu uso para o domínio espaço-temporal. Diferente das abordagens clássicas, esta gera CPEs que balanceiam a
coerência dos dados com a coerência espacial, mantendo \textit{features} similares próximas na linearização,
o que constitui uma estratégia promissora para reduzir a descontinuidade visual.


Com o objetivo de aprimorar a coerência visual da imagem resultante, foram propostas também técnicas de alinhamento
temporal entre os quadros, visto que a linearização independente de cada quadro pode introduzir descontinuidades,
como ilustrado na Figura \ref{fig:aligmnet_need}. A avaliação da proposta ocorre por meio de experimentos visuais
em animações sintéticas. Adicionalmente, este
trabalho implementa métricas de autocorrelação para aferir a coerência
espacial e dos dados na linearização resultante.


% Este trabalho traz uma implementação eficiente de fácil reuso e adaptação do método guiado por dados
% \textit{Data-Driven Space-Filling Curve} para imagens estáticas bem como uma expansão do seu uso para
% dados espaço-temporais, gerando imagens estáticas representativas do evento temporal como um todo.
% Este trabalho também implementa métricas avaliativas da coerência de dados e da coerência espacial
% para a linearização. Também é realizado experimentos visuais com animações criadas sinteticamente.
% Com o intuito de melhorar a coerência visual da imagem estática que representa todo o dado espaço-temporal,
% foi proposto formas de alinhamento neste trabalho também.
...
\begin{figure}[ht]
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{images/temporal_disalignment.png}
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{images/temporal_alignment.png}
        \caption{Com alinhamento}
    \end{subfigure}%
    \caption{Impacto do alinhamento temporal na linearização. Parte da animação detalhada na Figura \ref{subfig:moving_normal}.}
    \label{fig:aligmnet_need}
\end{figure}
...
% A visualização de dados temporais é um tópico explorado na literatura. Pe{\~n}a-Araya et al.~\cite{PenaAraya:20}
% compara diferentes formas de visualização para diferentes atividades, são elas: animações, pequenos múltiplos e
% glifos. Os resultados apontados nesse estudo indicam, entre outras coisas, que uma visão mais ampla de todo o evento
% com a técnica de pequenos múltiplos é a melhor de forma geral entre elas. Isso é um indicativo que uma abordagem que
% mostre vários dados temporais simultaneamente contribui para uma boa compreensão e análise, como com a linearização
% do espaço em uma dimensão e o tempo na outra.

A visualização de dados espaço-temporais é um tópico amplamente explorado na literatura. Peña-Araya et al.~\cite{PenaAraya:20}
conduziram estudos comparativos entre diferentes técnicas, especificamente: animações, pequenos múltiplos (\textit{small multiples})
e glifos. Os resultados indicam que, embora animações sejam eficazes para estimar direções, a visualização estática via pequenos
múltiplos apresenta melhor desempenho geral para tarefas de análise e comparação. Isso corrobora a relevância de abordagens que
permitem a visualização simultânea de múltiplos instantes temporais, tal qual a proposta de linearização do espaço em uma dimensão
com o tempo na outra.

% Curvas de preenchimento de espaço~\cite{sagan:94} são exploradas na literatura e David Hilbert inventou uma das mais famosas
% delas na Ciência da computação~\cite{hilbert:1891}. A maioria dessas curvas lidam mais com formatos geométricos sem ligar
% para o próprio dado em questão. Duas exceções a essa regra são as curvas definidas por Dafner et al.~\cite{dafner:00} e
% Zhou et al.\cite{zhou:20}, que propoem curvas data aware.

As curvas de preenchimento de espaço possuem fundamentos teóricos sólidos~\cite{sagan:94}, sendo a curva proposta por
David Hilbert~\cite{hilbert:1891} uma das mais famosas na Ciência da Computação. A maioria dessas curvas tradicionais é
construída com base em padrões geométricos fixos, sendo agnósticas ao conteúdo dos dados. Exceções importantes a essa regra
são as curvas definidas por Dafner et al.~\cite{dafner:00} e Zhou et al.~\cite{zhou:20}, que propõem abordagens guiadas por dados,
onde o caminho se adapta às características da imagem para preservar a coerência das informações. Enquanto Dafner et al. definem
a construção de caminhos hamiltonianos focados na maximização da coerência dos dados, Zhou et al. expandem esse conceito ao
introduzir mecanismos para balancear simultaneamente a coerência espacial.

% Usar linearizações do espaço para representar dados temporais foi explorado por Köpp e Weinkauf~\cite{KoeppWeinkauf2022_TMTM} e
% por Franke et al.~\cite{franke:21}. Köpp e Weinkauf desenvolveram métodos próprios de linearização, enquanto Franke et al. usa
% curvas de preenchimento como a de Hilbert, que não levam em conta o próprio conteúdo do dado.

Recentemente, o uso de linearizações espaciais para representar dados temporais foi investigado por Franke et al.~\cite{franke:21}
e por Köpp e Weinkauf~\cite{KoeppWeinkauf2022_TMTM}. Franke et al. utilizam projeções 1D baseadas em curvas clássicas, como a de
Hilbert, que não consideram a distribuição dos dados na imagem. Por outro lado, Köpp e Weinkauf desenvolvem um método
baseado em topologia. Nota-se, portanto, que a aplicação de curvas guiadas por dados especificamente para o contexto de visualização
de sequências temporais ainda é pouco explorada, motivando a abordagem deste trabalho.

\section{Metodologia}
% -----------------------------------------------------------------------------------------------
% -----------------------------------------------------------------------------------------------
% -----------------------------------------------------------------------------------------------
Nesta seção serão abordadas as técnicas implementadas e adaptadas neste trabalho para a
linearização e visualização de dados espaço-temporais.
...
A abordagem proposta fundamenta-se na linearização de estruturas multidimensionais (2D ou 3D) em sequências
unidimensionais (1D). Expandindo o conceito de CPEs baseadas em contexto
(\textit{Context-based Space-Filling Curves})~\cite{dafner:00}, utiliza-se a técnica de CPEs guiadas por
dados (\textit{Data-Driven Space-Filling Curves})~\cite{zhou:20}. O núcleo do processamento estabelece
um ciclo hamiltoniano que percorre integralmente a grade de pixels, com o objetivo de preservar
simultaneamente a coerência espacial, referente à localidade geométrica do caminho, e a coerência dos dados,
associada à localidade dos dados no caminho.

O processamento de dados espaço-temporais ocorre, inicialmente, pela linearização independente de cada quadro.
No entanto, curvas de preenchimento de espaço geradas de forma isolada para quadros consecutivos podem apresentar
variações topológicas abruptas, prejudicando a continuidade visual. Para mitigar esse problema, o sistema realiza
técnicas de alinhamento temporal, visando maximizar a correlação estrutural entre as curvas de quadros adjacentes.
O fluxograma completo do processo, incluindo a etapa de alinhamento, é apresentado na Figura \ref{fig:aligment_pipeline}.
...
\begin{figure}[ht]
    ...
    \begin{subfigure}[t]{0.35\textwidth}
        \centering
        \includegraphics[height=4.2cm, keepaspectratio]{images/pipeline/frames.png}
        \caption{Dado espaço-temporal} 
    \end{subfigure}%
    \hfill
    % --- Imagem 2 ---
    \begin{subfigure}[t]{0.31\textwidth}
        \centering
        \includegraphics[height=4.8cm, keepaspectratio]{images/pipeline/disaligment.png}
        \caption{Linearização}
    \end{subfigure}%
    % --- Imagem 3 ---
    \begin{subfigure}[t]{0.31\textwidth}
        \centering
        \includegraphics[height=4.8cm, keepaspectratio]{images/pipeline/aligment.png}
        \caption{Alinhamento}
    \end{subfigure}%
    \caption{Fluxograma do método proposto: (a) Entrada dos dados, (b) Linearização inicial e (c) Alinhamento temporal.}
    \label{fig:aligment_pipeline}
\end{figure}
...
\begin{itemize}
    ...
    \item \textbf{Dados espaço-temporais:} Investigação e avaliação de técnicas de alinhamento entre quadros para garantir a coerência temporal após a linearização individual de cada quadro.
\end{itemize}
...
Esta etapa detalha o funcionamento do método \textit{data-driven}, utilizado na linearização de cada quadro.
Diferentemente de curvas de preenchimento de espaço com geometria fixa, como a curva de Hilbert, este método
considera os valores dos dados durante a construção do caminho. O objetivo fundamental é maximizar a
autocorrelação na sequência unidimensional resultante, mantendo dados com valores semelhantes próximos na
linearização, sem sacrificar completamente a localidade espacial.

A compreensão do método \textit{data-driven} requer a análise preliminar da abordagem baseada em
contexto (\textit{Context-based}) proposta por Dafner et al.~\cite{dafner:00}. O conceito central fundamenta-se
no mapeamento da imagem original para o seu grafo dual, realizado através da construção de pequenos circuitos,
conforme ilustrado na Figura \ref{fig:small_circuits}. Após esse mapeamento, observa-se que uma árvore geradora
no grafo dual equivale a um ciclo hamiltoniano no grafo original. Dessa forma, busca-se definir uma função de peso
para as arestas do grafo dual que considere os valores dos pixels. Com essa função, constrói-se uma
Árvore Geradora Mínima (MST) no grafo dual para, subsequentemente, recuperar o ciclo hamiltoniano no grafo original.
Para a construção da MST, adotou-se o algoritmo de Prim.
...
A função de custo originalmente proposta é descrita na Equação \ref{eq:1}. Nela, $W(C_{i}, C_{j})$
representa o custo de conectar o vértice $C_{j}$ à árvore geradora, partindo de um vértice $C_{i}$
já pertencente a ela. As variáveis $(u, w, x, y, z, e, f)$ correspondem aos pesos das arestas no
grafo original que podem ser visualizados na Figura \ref{fig:context_weight}. O peso de cada aresta é definido
pela diferença absoluta entre os valores dos pixels em seus extremos. Segundo Dafner et al.~\cite{dafner:00},
para uma aresta $e=(a, b)$, tem-se $w(e)=|a - b|$. Para imagens coloridas, utiliza-se a soma das diferenças
absolutas de cada canal $c$: $w(e) = \sum_{c} |a_c - b_c|$.
...
Embora a abordagem \textit{context-based} gere linearizações que priorizam a coerência dos dados,
ela não considera explicitamente a coerência espacial. Isso pode ocasionar linearizações onde pixels
vizinhos na imagem original fiquem distantes na sequência 1D, como visto no exemplo da Figura \ref{fig:spc_traversal}.
A abordagem \textit{data-driven}, detalhada por Zhou et al.~\cite{zhou:20}, soluciona essa limitação introduzindo uma
função de custo composta, preservando a estrutura fundamental de mapeamento dual e MST proposta no
artigo original~\cite{dafner:00}.


A abordagem \textit{data-driven} utiliza a função de custo ponderada descrita na Equação \ref{eq:2}.
Os componentes da equação são definidos como:
...
Neste trabalho, $\alpha$ e $\beta$ foram definidos 
como hiperparâmetros configuráveis, permitindo o ajuste fino do comportamento da 
curva de preenchimento de espaço e a análise do impacto que a variação desses parâmetros exerce sobre a 
...
Neste trabalho, foram avaliadas duas métricas distintas para a função g: a Norma $L_1$ e a Norma $L_2$.
Para ambas, são funções facilmente extensíveis para imagens com múltiplos canais.

\subsubsection{Norma $L_1$}
% Explicar a função g para Norma $L_1$ e tempo de complexidade
...
\subsubsection{Norma $L_2$}
% Explicar a função g para Norma $L_2$ e tempo de complexidade
...
Analogamente, dados espaço-temporais são processados como uma sequência de imagens estáticas do tipo $F \times H \times W$ ou $F \times H \times W \times C$,
onde os parâmetros são definidos como:

\begin{itemize}
    \item $F$: quantidade de quadros;
    \item $H$: altura da imagem (número de linhas);
    \item $W$: largura da imagem (número de colunas);
    \item $C$: número de canais de cor.
\end{itemize}

Visando alto desempenho computacional, o núcleo de processamento foi implementado inteiramente em C++20.
Para garantir interoperabilidade e facilitar a análise exploratória, as funcionalidades foram expostas ao ambiente
Python utilizando a biblioteca \texttt{Pybind11}, que atua como uma camada de integração leve. O ambiente de
validação experimental foi construído sobre \textit{Jupyter Notebooks}, permitindo prototipagem
...
O código-fonte desenvolvido encontra-se disponível publicamente no repositório: \url{https://github.com/gabmei/spatio-temporal-data-driven-sfc}.
...
Ressalta-se que, para garantir a consistência nos experimentos, todos os valores foram normalizados
para valores em ponto flutuante no intervalo $[0.0, 1.0]$.

\subsubsection{Imagens de Teste}
Para a validação da etapa de linearização, foram adotadas as figuras sintéticas propostas por
...
Também para a validação da etapa de linearização, foram utilizados diversos
volumes de dados disponíveis na coleção
...
Inicialmente, analisa-se a sensibilidade da implementação \textit{data-driven} à variação de seus hiperparâmetros.
...
Em seguida, compara-se a abordagem proposta com as curvas de Hilbert, \textit{scanline} e \textit{context-based}.
As duas primeiras possuem geometria fixa: a de Hilbert baseia-se em recursão fractal, enquanto a \textit{scanline}
segue uma varredura sequencial liha a linha. 
Para este comparativo, foi adotada a
imagem \textit{Nucleon} da Figura \ref{fig:openscivis_slices}(f), um dataset de referência frequentemente utilizado
para validação de linearizações em trabalhos relacionados, como os de Zhou et al.~\cite{zhou:20} e Köpp e
...
% autocorrelação normalizada de duas métricas para analisar o desempenho das CPE. A normalização dos
...
% Para quantificar a qualidade das curvas de preenchimento de espaço, replicam-se os critérios estabelecidos por
% Zhou et al.~\cite{zhou:20}, que avaliam a capacidade da curva em preservar a localidade dos dados e espacial após
% a linearização. A principal ferramenta adotada para isso é a autocorrelação, uma medida que indica o grau de
% dependência linear entre uma sequência de dados e uma versão deslocada de si mesma. Dada uma sequência
% unidimensional $x[n]$ de comprimento $N$, a autocorrelação $R_{xx}[k]$ para um deslocamento $k$ é definida
% matematicamente pela correlação cruzada na Equação \ref{eq:autocorr}. Esta sequência está definida para
% $k \in [-(N-1), +(N-1)]$. Para normalizar os valores resultantes da
% autocorrelação no intervalo $[-1, +1]$ se faz $x \leftarrow \frac{x}{\|x\|}$ anteriormente, tal que quanto maior os valores, mais
% semelhantes são os dados para um deslocamento $k$. Dessa forma permitindo a
% comparação justa entre diferentes conjuntos de dados. O objetivo final é criar plots de figuras onde o eixo x está o
% deslocamento $k$, e o eixo y representa a média dos valores encontrados no intervalo $[-k, +k]$ em $R_{xx}$.

Para quantificar a qualidade das curvas de preenchimento de espaço, replicam-se os critérios estabelecidos por
Zhou et al.~\cite{zhou:20}, os quais avaliam a capacidade da curva em preservar tanto a localidade dos dados
quanto a localidade espacial após a linearização. A principal ferramenta adotada para isso é a
autocorrelação, uma medida que indica o grau de similaridade entre uma sequência de dados
e uma versão deslocada de si mesma.

Dada uma sequência unidimensional discreta $x[n]$ de comprimento $N$, a autocorrelação $R_{xx}[k]$ para um
deslocamento $k$, definido no intervalo de $k$ entre $-(N-1)$ e $+(N-1)$, é calculada conforme a Equação \ref{eq:autocorr}.
Para assegurar a invariância de escala e permitir uma comparação justa entre diferentes conjuntos de dados, a
sequência $x$ é previamente normalizada pela sua norma euclidiana $L_2$. Com essa normalização, os valores
resultantes de $R_{xx}$ restringem-se ao intervalo $[-1, +1]$, onde valores próximos de $+1$ indicam alta similaridade
para o deslocamento $k$. A métrica final é visualizada graficamente, onde o eixo horizontal representa o deslocamento
$k$ e o eixo das vertical exibe a média dos valores de autocorrelação computados na janela $[-k, +k]$.

\begin{equation} \label{eq:autocorr}
    \begin{split}
        x & \leftarrow \frac{x}{\|x\|} \\
        R_{xx}[k] & = \sum_{i=0}^{N-1} x[i] \cdot x[i+k]
    \end{split}
\end{equation} 


Duas sequências são construídas a partir da linearização $P$, onde $P_i$ representa a i-ésima coordenada visitada
na curva de preenchimento de espaço. A partir delas, são construídas as métricas de autocorrelação utilizadas. São elas:

\begin{itemize}
 \item \textbf{Autocorrelação Média de Dados:} Avalia se pixels com intensidades semelhantes na imagem permanecem próximos na sequência linearizada.
 Seja $I(p)$ a intensidade do pixel na posição $p$, a sequência é definida como $s[i] = I(P_i)$.
 \item \textbf{Autocorrelação Média Radial:} Avalia se coordenadas próximas permanecem próximas na sequência linearizada.
 A sequência é definida como $s[i] = \|P_i\|$, medindo a distância da coordenada $P_i$ à origem.
\end{itemize}
...
abordagens \textit{context-based}, Hilbert e \textit{scanline} implementados neste trabalho. A avaliação
...
Representações da linearização resultante são exibidas nas linhas \textbf{(b)}, \textbf{(c)} e \textbf{(d)}.
A linha \textbf{(b)} exibe o sinal unidimensional gerado, onde o eixo vertical corresponde ao valor do pixel e
o eixo horizontal ao índice da sequência linearizada.
Complementarmente, as linhas \textbf{(c)} e \textbf{(d)} ilustram
o trajeto da curva de preenchimento de espaço sobre a imagem através de um gradiente visual, onde tons
escuros indicam o início e tons claros, o fim do percurso.
Observa-se em \textbf{(b)} que tanto a abordagem
\textit{data-driven} quanto a \textit{context-based} conseguem capturar a semântica dos dados, traduzindo os
dois picos originais em elevações contínuas e distintas no gráfico. Em contrapartida, as curvas Hilbert
e \textit{scanline}, por serem agnósticas aos dados, fragmentam essas estruturas, gerando um sinal oscilatório que
não reflete diretamente a estrutura do dataset. Essa distinção comportamental é corroborada pelavisualização do caminho
...
\begin{itemize}
    ...
    \item Na abordagem \textbf{\textit{data-driven}}, a introdução do custo de coerência espacial
...
precedência das direções (norte, sul, leste, oeste) atua como critério de desempate, induzindo a CPE em direções preferenciais.
...
transversais de cada volume do \textit{Open SciVis} presentes na Tabela \ref{tab:openscivis}, juntos a 10 quadros das animações
...
configurações avaliadas. Os resultados comparativos dessa avaliação são detalhados nas
Figuras \ref{fig:autocorrelation_alpha_results} e \ref{fig:autocorrelation_algorithms_results}. 
...
\begin{figure}[htbp]
    ...
    Hilbert e \textit{scanline}. (a) A métrica de dados avalia a
    continuidade das intensidades. (b) A métrica radial avalia a preservação da vizinhança espacial.}
...
% se compara contra as abordagens Hilbert e \textit{scanline} para o valor fixo de $\alpha = 0.1$ nas métricas
...
\begin{figure}[htbp]
    ...
    \makebox[0.32\linewidth]{\textbf{Norma $L_1$}} \hfill
    \makebox[0.32\linewidth]{\textbf{Norma $L_2$}}
    \par\smallskip
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[c]{0.60\linewidth}
        ...
        \caption{Rotação do resultado da Norma $L_2$ da Figura \ref{fig:anim_expanding_ring}}
...
% a complexidade da estratégia de alinhamento Norma $L_1$, que possui complexidade quadrática no número de pixels, enquanto
% a estratégia Norma $L_2$ introduz pouco custo adicional, pois tem a mesma complexidade de execução do algoritmo de Prim.
...
Observa-se o elevado custo da estratégia Norma $L_1$, que exibe complexidade quadrática em relação ao
número de pixels, enquanto a estratégia Norma $L_2$ introduz pouco custo adicional, pois possui a mesma
...
Os experimentos confirmam o \textit{trade-off} imposto pelo fator de regularização espacial
...
Norma $L_1$ e Norma $L_2$. Os resultados indicam a necessidade dessas técnicas, uma vez que sua
